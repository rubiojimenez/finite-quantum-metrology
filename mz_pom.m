...
elseif pom_choice==5
  ...
  paritya=sparse(kron(identity(op_cutoff),(-1)ˆ(full(creation(op_cutoff)*creation(op_cutoff)’))));
  parityb=sparse(kron((-1)ˆ(full(creation(op_cutoff)*creation(op_cutoff)’)),identity(op_cutoff)));
